...
<body>
	<%
		...
		Iterator downiter = list.iterator();
	%>
...
<div class="user-home clear">
	<div class="user-nav">
		<ul>
			...
			<li>
				<a href="userdownload?index=1" class="active">
	...
	<div class="user-home-content">
		...
		<div class="user-home-contain">
			<div id="masonry" class="container-fluid clear">
		  	<% if(downiter.hasNext()==false)
				{%>下载任何作品，快去下载吧！<%}
			%>
			<%
				while (downiter.hasNext()) {
				Work work = (Work) downiter.next();
			%>
		  		<div class="box">
		  			...
		  			<a href="show.onework?id=<%=work.getId()%>" class="mask"></a>
			  			...
			  			<div class="activeProporty clear">
			  				<a class="downloadNum">
			  					<span class="downloadNumText">已下载<span style="color:#b51d1a;font-size:13px;margin:0 5px;"><%=work.getDownload()%></span>次</span>
			  				</a>
			  			</div>
		  			</div>
		  		</div>
		...
		<table class="page_table" border="0" cellspacing="0" cellpadding="0">  
             <tr>  
               ...
               <td width="40" align="right"><a href="userdownload?index=1" ><span >首页</span></a></td>
               <% if(pages.getCurrentPage()>1){ %>  
               <td width="40" align="right"><a href="userdownload?index=${indexs-1}&ticket=getNewXsgl"> <span >上一页</span></a></td>  
               <% }%>
               <% if(pages.getCurrentPage()==1){ %>  
               <td width="40" align="right"><a href="userdownload?index=1&ticket=getNewXsgl"><span >上一页</span></a></td>  
               <% }%>
               ...
               <td width="40" align="left"><a href="userdownload?index=${indexs+1}&ticket=getNewXsgl"><span >下一页</span></a></td>  
               <%} %>
               <% if(pages.getCurrentPage()==pages.getPageTotal()){ %>  
               <td width="40" align="left"><a href="userdownload?index=<%=pages.getPageTotal() %>&ticket=getNewXsgl"><span  >下一页</span></a></td>  
               <%} %>
               <td width="40" align="left"><a href="userdownload?index=<%=pages.getPageTotal() %>&ticket=getNewXsgl"><span >尾页</span></a></td>
             </tr>
        </table>
       <!--  <form action="userdownload">
