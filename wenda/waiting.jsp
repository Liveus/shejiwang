...
<body>
	<%
		...
		PageBean pages = (PageBean) session.getAttribute("solveQuestions");
   		List list = pages.getList();
		Iterator iter = list.iterator();
		String interests = (String)session.getAttribute("interests");
		String inters = (String)session.getAttribute("inters");
...
<div class="contain">
	<div class="wenda clear">
		<div class="wenda-main fl">
			<div class="nav">
				<a href="index2.question?QAindex=1" >推荐</a>
				<a href="time.question?QTindex=1">最新</a>
				<a href="waiting.question?QSindex=1" class="active">等待回答</a>
			</div>
			...
			<%
			 ...
			 %>
				...
				<table class="page_table" border="0" cellspacing="0" cellpadding="0">  
		             <tr>
		               <td width="40" align="right"><a href="waiting.question?QSindex=1" ><span >首页</span></a></td>
		               <% if(pages.getCurrentPage()>1){ %>
		               <td width="40" align="right"><a href="waiting.question?QSindex=${QSindexs-1}&ticket=getNewXsgl"> <span >上一页</span></a></td>  
		               <% }%>
		               <% if(pages.getCurrentPage()==1){ %>
		               <td width="40" align="right"><a href="waiting.question?QSindex=1&ticket=getNewXsgl"><span >上一页</span></a></td>  
		               <% }%>
		               ...
		               <td width="40" align="left"><a href="waiting.question?QSindex=${QSindexs+1}&ticket=getNewXsgl"  ><span >下一页</span></a></td>  
		               <%} %>
		               <% if(pages.getCurrentPage()==pages.getPageTotal()){ %>  
		               <td width="40" align="left"><a href="waiting.question?QSindex=<%=pages.getPageTotal() %>&ticket=getNewXsgl"><span  >下一页</span></a></td>  
		               <%} %>
		               <td width="40" align="left"><a href="waiting.question?QSindex=<%=pages.getPageTotal() %>&ticket=getNewXsgl"><span >尾页</span></a></td>
		             </tr>
		        </table>
			</div>
		</div>
		<div class="wenda-slider fr">
			...
			<div class="user-about">
				<div class="user-info">
				<%if(interests.equals("0")==false) {%>
			        <div class="user-pic">
			            <a href="/u/1301488/bbs">
			                <img src="<%=basePath %>/user/head/${user.head}" alt="仲夏1">
			            </a>
			        </div>
			        <div class="user-name">
			            <a href="/u/1301488/bbs">${user.name}</a>
			                                </div>
			        <div class="integral-info clearifx">
			            <a href="/u/1301488/credit" class="integral">积分：${user.integral}</a>
